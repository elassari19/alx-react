...
describe('<CourseListRow />', () => {
	it(`is true,should renders on cell with colspan=2
	when textSecondCell does not exist`, () => {
		const wrapper = shallow(<CourseListRow isHeader={true} textFirstCell="test" />);
		expect(wrapper.find('th').exists()).toBe(true);
	})

	it(`is true,should renders two cells when textSecondCell is present`, () => {
		const wrapper = shallow(<CourseListRow isHeader={true} textFirstCell="test" textSecondCell="test2" />);
		expect(wrapper.find('th').length).toBe(2);
	})

	it(`is false,should renders two td elements within a tr element`, () => {
